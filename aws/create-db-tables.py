...
def lambda_handler(event, context):
    ...
    """
    try:
        conn = pymysql.connect(host=rds_host, port=3306, user=user_name, passwd=password, db=db_name, connect_timeout=10)
    ...
    order_sql = "create table if not exists CUSTOMER_ORDER (ID int auto_increment primary key, CUSTOMER_NAME varchar(255), SHIPPING_INFO_TOKEN int, PAYMENT_INFO_TOKEN int, STATUS varchar(255) default 'New', ORDER_TIME datetime)"
    ...
    shipping_sql = "create table if not exists SHIPPING_INFO (ID int auto_increment primary key, CONFIRMATION_TOKEN int, ADDRESS1 varchar(255), ADDRESS2 varchar(255), CITY varchar(255), STATE varchar(255), POSTAL_CODE varchar(255))"
